...
#define I2C2_SDA	12		// PA12 - AF5
#define I2C2_SCL	11		// PA11 - AF5
...
void init_i2c(void) {
	...
	/* select alternate function mode */
	GPIOB->MODER = (GPIOA->MODER & ~(GPIO_MODER_MODER11 | GPIO_MODER_MODER12))
			| (GPIO_MODER_MODER11_1 | GPIO_MODER_MODER12_1);

	GPIOB->OTYPER |= (GPIO_OTYPER_OT_11 | GPIO_OTYPER_OT_12);

	GPIOB->OSPEEDR &= ~(GPIO_OSPEEDER_OSPEEDR11_0 | GPIO_OSPEEDER_OSPEEDR12_0);

	GPIOB->PUPDR &=    ~(GPIO_PUPDR_PUPDR11 | GPIO_PUPDR_PUPDR12);
